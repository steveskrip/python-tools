...
l2, = ax.plot_date(df.index.to_pydatetime(),df['FNA 1973'],'-',label='FNA 1973')
l2, = ax.plot_date(df.index.to_pydatetime(),df['FNA 1985'],'-',label='FNA 1985')
l2, = ax.plot_date(df.index.to_pydatetime(),df['FNA 1987'],'-',label='FNA 1987')
l3, = ax.plot_date(df.index.to_pydatetime(),df['FNA 1993'],'-',label='FNA 1993')
l2, = ax.plot_date(df.index.to_pydatetime(),df['FNA 2000'],':',label='FNA 2000')
l1, = ax.plot_date(df.index.to_pydatetime(),df['FNA 1977'],'-',label='FNA 1977',color='darkgrey')
l4, = ax.plot_date(df.index.to_pydatetime(),df['FNA 50%-ile'],'D',color='darkgrey',label='FNA 50%-ile')
...
ax.yaxis.grid()
ax.legend()
plt.title('FNA Flow Comparison - Truckee River '+LOCAT+'\n')
plt.xlabel('Month')
plt.ylabel('Flow (cfs)')
plt.tight_layout()

#wrap it up
plt.savefig(dir+"/FNAPlots_BestYears.png",dpi=300)
plt.savefig(dir+"/FNAPlots_BestYears.pdf",dpi=300)
plt.show()
