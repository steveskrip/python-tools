...
def get_ptile(dates,nfile,PTILEHEADER):
    df_ptile = pd.read_csv(cdir(nfile), index_col=0, parse_dates=True)
    values = df_ptile[PTILEHEADER].ix[dates].values
    print df_ptile
    return values
...
l4, = ax.plot_date(df.index.to_pydatetime(),df['TROA 50%-ile'],'D',color='darkgrey',label='TROA 50%-ile')
...
#wrap it up
plt.savefig(dir+"/TROAPlots_BestYears.png",dpi=300)
plt.savefig(dir+"/TROAPlots_BestYears.pdf",dpi=300)
plt.show()
